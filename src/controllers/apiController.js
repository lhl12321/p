...
exports.addbrand = (req, res) => {
  ...
  var sql = 'insert into dt_brands (name) values (?)'
...
exports.adduser = (req, res) => {
  ...
  // var sql = 'insert into dt_brands (name, ctime) values ("' + info.name + '", "' + new Date() + '")';
  var sql = 'insert into dt_users (user_name, password, mobile, nick_name) values (?, ?, ?, ?)'
  req.db.driver.execQuery(sql, [info.user_name, info.password, info.mobile, info.nick_name, new Date()], (err, data) => {
    if (err) {
      resObj.status = fialState
      resObj.message = err.message
      res.end(JSON.stringify(resObj))
...
exports.getusername = (req, res) => {
   ...
   */
  let sql = " SELECT id,user_name,password FROM dt_users "
  console.log('获取用户名和密码sql语句：============>', sql)
